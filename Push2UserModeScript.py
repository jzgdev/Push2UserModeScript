...
class Push2UserModeScript(ControlSurface):
    __doc__ = " Push 2 User Mode Script "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in Push2UserModeScript._active_instances:
    ...
    def _do_combine(self):
        if self not in Push2UserModeScript._active_instances:
            Push2UserModeScript._active_instances.append(self)
            Push2UserModeScript._combine_active_instances()


    def _do_uncombine(self):
        if ((self in Push2UserModeScript._active_instances) and Push2UserModeScript._active_instances.remove(self)):
    ...
    def _setup_session_control(self):
        ...
        self._scene_launch_buttons = [self._ctrl_map[SCENELAUNCH[index]] for index in range(CONTROL_RING_HEIGHT)]
        self._track_stop_buttons = [self._note_map[TRACKSTOP[index]] for index in range(CONTROL_RING_WIDTH)]
        ...
        for scene_index in range(CONTROL_RING_HEIGHT):
            scene = self._session.scene(scene_index)
            scene.name = 'Scene_' + str(scene_index)
            button_row = []
            scene.set_launch_button(self._scene_launch_buttons[scene_index])
            scene.set_triggered_value(2)
            for track_index in range(CONTROL_RING_WIDTH):
